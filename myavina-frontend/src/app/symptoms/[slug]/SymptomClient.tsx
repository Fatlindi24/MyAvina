// src/app/symptoms/[slug]/SymptomClient.tsx
"use client";
...
export default function SymptomClient({
  ...
  // Build sections dynamically, but memoize so the reference is stable for useEffect deps
  const sections = useMemo(
    ...
    [s.redFlags.length, s.faq.length]
  );
            ...
            <div className="mt-12">
              ...
              <div className="grid grid-cols-1 gap-4">
                {recommended.map((p) => (
                  <Link
                    key={p.id}
                    href={`/products/${p.slug}`} // ✅ corrected internal route
