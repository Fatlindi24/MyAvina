...
const ctaData = [
  {
    ...
    description: "Connect with MyAvina's menopause specialists and get real-time answers to all your questions.",
  ...
  },
  {
    ...
    description: "Learn what's normal and what to expect, so you're never caught off guard.",
...
const CtaCard: React.FC<CtaCardProps> = ({ title, description, buttonText, buttonLink, imageUrl, buttonVariant = 'dark' }) => {
  
  const buttonClasses = buttonVariant === 'dark'
    ? 'bg-black text-white hover:bg-gray-800'
    : 'bg-white text-black border border-transparent hover:bg-gray-200';

  return (
    <Link href={buttonLink} className="group relative block w-full aspect-square md:aspect-[4/3] rounded-2xl overflow-hidden shadow-lg">
      ...
      <div className="relative h-full flex flex-col justify-between p-8 sm:p-10 text-white">
        ...
        {/* Bottom Button */}
        <div>
          <span className={`inline-block font-normal text-[16px] leading-[100%] tracking-normal px-6 py-3 rounded-full transition-colors ${buttonClasses}`}>
            {buttonText}
          </span>
        </div>
